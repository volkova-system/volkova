// Package data provides an in-memory caching layer for queue data using BuntDB.
// It offers efficient storage and retrieval of queue information with indexing
...
// defaultCacheName is the default index name used for queue storage.
//
const defaultCacheName = "queues"

// Cache represents an in-memory cache for queue data using BuntDB.
//
type Cache struct {
	name        string
	queues      *buntdb.DB
