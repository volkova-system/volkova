...
func RunControl() {
	server := fiber.New(fiber.Config{
		...
		ErrorHandler: func(c fiber.Ctx, err error) error {
			...
			log.Printf("runtimes data control service error [%d]: %v", code, err)
	...
	routes.RegisterRuntimesControlRoutes(runtimesGroup)
	...
	go func() {
		...
		if useFD {
			...
			if file != nil {
				...
				if err != nil {
					...
					log.Printf("runtimes data control service descriptor listener error: %v", err)
		...
		if err := server.Listen(":" + port); err != nil {
			log.Printf("runtimes data control service listen error: %v", err)
	...
	if err := server.Shutdown(); err != nil {
		log.Printf("runtimes data control service shutdown error: %v", err)
	}
}
