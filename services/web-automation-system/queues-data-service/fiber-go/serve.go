...
// ServeData opens the queue cache and starts the data HTTP server.
//
// The data server exposes CRUD endpoints for queue records and runs
...
func ServeData() {
	...
	server := fiber.New(fiber.Config{
		...
		ErrorHandler: func(c fiber.Ctx, err error) error {
			...
			log.Printf("queues data service error [%d]: %v", code, err)
	...
	routes.RegisterQueuesDataRoutes(queuesGroup, cache)
	...
	go func() {
		...
		if useFD {
			...
			if file != nil {
				...
				if err != nil {
					...
					log.Printf("queues data service descriptor listener error: %v", err)
		...
		if err := server.Listen(":" + port); err != nil {
			log.Printf("queues data service listen error: %v", err)
	...
	select {
	case <-ctx.Done():
		log.Println("received terminate signal, shutting down queues data service...")
	case <-channels.GetShutdownChannel():
		log.Println("received shutdown request, shutting down queues data service...")
	case <-channels.GetAbortChannel():
		log.Println("received abort request, shutting down queues data service...")
	}

	if err := server.Shutdown(); err != nil {
		log.Printf("queues data service shutdown error: %v", err)
	}
}
