...
func RunParent() {
	...
	if runtime.GOOS != "windows" {
		...
		for {
			...
			cmd.Env = append(os.Environ(), "JOBS_DATA_SERVICE_USE_FD=1", "USE_INHERITED_FD=1", "SOCKET_FD=3")
			...
			if code == abortExitCode {
				...
				for {
					...
					ctrl.Env = append(os.Environ(), "JOBS_DATA_SERVICE_USE_FD=1", "USE_INHERITED_FD=1", "SOCKET_FD=3", "JOBS_DATA_SERVICE_MODE=control")
		...
		for {
			...
			if code == abortExitCode {
				...
				for {
					...
					ctrl.Stdout = os.Stdout
					ctrl.Stderr = os.Stderr
					ctrl.Env = append(os.Environ(), "JOBS_DATA_SERVICE_MODE=control")
					if err := ctrl.Start(); err != nil {
						log.Printf("control child start error: %v", err)
						time.Sleep(3 * time.Second)
...
func RunChild() {
	if os.Getenv("JOBS_DATA_SERVICE_MODE") == "control" {
		RunControl()
