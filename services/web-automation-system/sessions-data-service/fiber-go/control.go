...
func RunControl() {
	server := fiber.New(fiber.Config{
		...
		ErrorHandler: func(c fiber.Ctx, err error) error {
			...
			log.Printf("sessions data control service error [%d]: %v", code, err)
	...
	routes.RegisterSessionsControlRoutes(sessionsGroup)
	...
	go func() {
		...
		if useFD {
			...
			if file != nil {
				...
				if err != nil {
					...
					log.Printf("sessions data control service descriptor listener error: %v", err)
		...
		if err := server.Listen(":" + port); err != nil {
			log.Printf("sessions data control service listen error: %v", err)
	...
	if err := server.Shutdown(); err != nil {
		log.Printf("sessions data control service shutdown error: %v", err)
	}
}
