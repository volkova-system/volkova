...
// ServeData opens the session cache and starts the data HTTP server.
//
// The data server exposes CRUD endpoints for session records and runs
...
//  2. Port from SESSIONS_DATA_SERVICE_PORT env var.
...
func ServeData() {
	...
	server := fiber.New(fiber.Config{
		...
		ErrorHandler: func(c fiber.Ctx, err error) error {
			...
			log.Printf("sessions data service error [%d]: %v", code, err)
	...
	routes.RegisterSessionsDataRoutes(sessionsGroup, cache)
	...
	go func() {
		useFD := os.Getenv("SESSIONS_DATA_SERVICE_USE_FD") == "1" ||
		...
		if useFD {
			...
			if file != nil {
				...
				if err != nil {
					...
					log.Printf("sessions data service descriptor listener error: %v", err)
		...
		if err := server.Listen(":" + port); err != nil {
			log.Printf("sessions data service listen error: %v", err)
	...
	select {
	case <-ctx.Done():
		log.Println("received terminate signal, shutting down sessions data service...")
	case <-channels.GetShutdownChannel():
		log.Println("received shutdown request, shutting down sessions data service...")
	case <-channels.GetAbortChannel():
		log.Println("received abort request, shutting down sessions data service...")
	}

	if err := server.Shutdown(); err != nil {
		log.Printf("sessions data service shutdown error: %v", err)
	}
}
