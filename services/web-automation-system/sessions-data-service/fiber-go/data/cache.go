// Package data provides an in-memory caching layer for session data using BuntDB.
// It offers efficient storage and retrieval of session information with indexing
...
// defaultCacheName is the default index name used for session storage.
//
const defaultCacheName = "sessions"

// Cache represents an in-memory cache for session data using BuntDB.
//
type Cache struct {
	name        string
	sessions    *buntdb.DB
...
func Open() (*Cache, error) {
    dbPath := os.Getenv("SESSIONS_CACHE_PATH")
    ...
    if dbPath == "" {
        ...
        dbPath = filepath.Join(absPath, "sessions-cache.db")
    }
...
func OpenWithPath(dbPath string) (*Cache, error) {
	...
	return &Cache{name: defaultCacheName, sessions: conn,
...
func (db *Cache) Close() error {
	if db.path != "" {
		...
		if err := db.sessions.Save(f); err != nil {
			return fmt.Errorf(
                "failed to save db snapshot: %w", err)
		}
	}

	return db.sessions.Close()
}
