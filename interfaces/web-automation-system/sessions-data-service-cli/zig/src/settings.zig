...
// default sessions route prefix
pub const route_prefix: []const u8 = "/service/data/sessions";
...
// load reads SESSIONS_DATA_SERVICE_HOST and
// SESSIONS_DATA_SERVICE_PORT from the environment.
...
pub fn load(allocator: std.mem.Allocator) Setting {
    const host = std.process.getEnvVarOwned(
        allocator,
        "SESSIONS_DATA_SERVICE_HOST",
    ) catch default_host;

    const port = std.process.getEnvVarOwned(
        allocator,
        "SESSIONS_DATA_SERVICE_PORT",
    ) catch default_port;
