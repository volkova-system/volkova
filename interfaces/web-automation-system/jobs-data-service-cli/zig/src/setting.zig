...
pub const default_port: []const u8 = "4073";

// default jobs route prefix
pub const route_prefix: []const u8 = "/service/data/jobs";
...
// load reads JOBS_DATA_SERVICE_HOST and
// JOBS_DATA_SERVICE_PORT from the environment.
...
pub fn load(allocator: std.mem.Allocator) Setting {
    const host = std.process.getEnvVarOwned(
        allocator,
        "JOBS_DATA_SERVICE_HOST",
    ) catch default_host;

    const port = std.process.getEnvVarOwned(
        allocator,
        "JOBS_DATA_SERVICE_PORT",
    ) catch default_port;
