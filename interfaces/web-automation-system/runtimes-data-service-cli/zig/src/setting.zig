...
pub const default_port: []const u8 = "4074";

// default runtimes route prefix
pub const route_prefix: []const u8 = "/service/data/runtimes";
...
// load reads RUNTIMES_DATA_SERVICE_HOST and
// RUNTIMES_DATA_SERVICE_PORT from the environment.
...
pub fn load(allocator: std.mem.Allocator) Setting {
    const host = std.process.getEnvVarOwned(
        allocator,
        "RUNTIMES_DATA_SERVICE_HOST",
    ) catch default_host;

    const port = std.process.getEnvVarOwned(
        allocator,
        "RUNTIMES_DATA_SERVICE_PORT",
    ) catch default_port;
