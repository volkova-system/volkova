...
// load reads TASKS_DATA_SERVICE_HOST and
// TASKS_DATA_SERVICE_PORT from the environment.
...
pub fn load(allocator: std.mem.Allocator) Setting {
    const host = std.process.getEnvVarOwned(
        allocator,
        "TASKS_DATA_SERVICE_HOST",
    ) catch default_host;

    const port = std.process.getEnvVarOwned(
        allocator,
        "TASKS_DATA_SERVICE_PORT",
    ) catch default_port;
