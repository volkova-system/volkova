...
<#
.SYNOPSIS
    Starts the TerminusDB Docker container.

.DESCRIPTION
    Starts the TerminusDB Docker container using docker-compose. The
    script validates prerequisites, checks container state, and ensures
    graceful startup with proper error handling and logging.
...
.NOTES
    ...
    Requirements: pwsh 7.6.0, Docker, docker-compose
...
function Start-Container {
    <#
    .SYNOPSIS
        Executes docker-compose up with proper error handling.

    .DESCRIPTION
        Runs docker-compose up -d from the compose directory. Captures
        output, checks exit code, and handles failures appropriately.
    ...
    .EXAMPLE
        ...
        if ($exitCode -eq 0) {
            Write-InfoLog -Scope "CONTAINER-START" -Message "Started"
        }
    #>
    [CmdletBinding()]
    [OutputType([int])]
...
try {
    ...
    Write-InfoLog -Scope "START-SCRIPT" `
        -Message "TerminusDB container started successfully"
