...
proc executeCommand(command: string, args: seq[string]) =
  ...
  of "install-nim":
    ...
    let dirValidation = handler.validateToolDirectory(
      cmdValidation.toolDirectory
    )

    if not dirValidation.valid:
      stderr.writeLine("Error: ", dirValidation.errorMsg)
      quit(2)
    ...
    try:
      ...
      let toolName = lastPathPart(cmdValidation.toolDirectory)
