...
proc executeCommand(command: string, parameters: seq[string]) =
    ...
    of "install-fiber-go":
        ...
        if not session.status:
            stderr.writeLine("install-fiber-go issue, ", session.issue)
            printUsage()
            quit(1)
        ...
        try:
            ...
            stderr.writeLine("install-fiber-go issue, ", issue.msg)
            quit(2)
        ...
        echo "install-fiber-go done, target, " & session.target
        quit(0)
