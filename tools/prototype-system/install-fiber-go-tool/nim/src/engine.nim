# Core engine for install-fiber-go tool operations

import os, osproc, strutils
import utils

proc getCurrentPlatform*(): string =
    ## Get current platform name matching build-fiber-go output directory
...
proc resolveExecutablePath*(serviceName: string): string =
    ## Resolve the expected executable path within the service directory
    ## Args: serviceName - Relative service path 'name-system/name-service'
    ## Returns: Absolute path to the expected executable file
    ##
    let serviceDir = utils.resolveServiceDir(serviceName)
    let serviceNameOnly = lastPathPart(serviceName)
    let platform = getCurrentPlatform()
    let execName = serviceNameOnly & getExecutableExtension()
    let execPath = absolutePath(
        serviceDir / "fiber-go" / "dist" / platform / execName
    )
    return execPath
...
proc getInstallDirectory*(): string =
    ...
    ##
    when defined(windows):
        return absolutePath(getEnv("ProgramFiles") / "fiber-go-tools" / "bin")
    elif defined(linux):
