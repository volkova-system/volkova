# Command handler for build-zig tool
...
proc validateCliStructure*(cliName: string,
    ...
    if not dirExists(srcDir):
        return ValidationResult(
          valid: false,
          errorMsg: "Source directory not found in CLI: " & cliName & "/zig/src"
        )

    # Validate main.zig exists
    if not fileExists(mainFile):
        return ValidationResult(
          valid: false,
          errorMsg: "Main file not found in CLI: " & cliName & "/zig/src/main.zig"
        )
...
proc validateCommand*(cmd: string, args: seq[string]): ValidationResult =
    ## Validate build-zig command and arguments
    ## Args: cmd - Command name (should be "build-zig")
