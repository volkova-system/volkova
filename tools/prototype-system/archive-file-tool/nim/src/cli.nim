...
proc executeCommand(command: string, parameters: seq[string]) =
    ...
    of "archive-file":
        ...
        if not session.status:
            stderr.writeLine("archive-file issue, ", session.issue)
            printUsage()
            quit(1)
        ...
        try:
            session = engines.archiveFile(session)
        except CatchableError as issue:
            stderr.writeLine("archive-file issue, ", issue.msg)
            quit(2)

        session = handlers.validateTargetArchivedFile(session)
        if not session.status:
            stderr.writeLine("archive-file issue, ", session.issue)
            quit(1)

        echo "archive-file done, target, " & session.target
        quit(0)

    else:
        stderr.writeLine(
            "archive-file issue, unknown command, '" & command & "'"
        )
        quit(1)
proc run*() =
    ...
    if targetCommand == "":
        stderr.writeLine(
            "archive-file issue, invalid command, '" & command & "'"
        )
        printUsage()
        quit(1)
